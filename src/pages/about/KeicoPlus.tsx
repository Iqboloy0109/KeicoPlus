...
export default function KeicoPlus() {
  ...
  const { ref: mainDescriptionRef, inView: mainDescriptionInView } = useInView({
    threshold: 0.2,
    triggerOnce: false,
  });

  const { ref: businessAreasRef, inView: businessAreasInView } = useInView({
    threshold: 0.1,
    triggerOnce: false,
  });
  ...
  useEffect(() => {
    const updateActiveSection = () => {
      if (mainDescriptionInView) {
        setActiveSection("section1");
      } else if (businessAreasInView) {
        setActiveSection("section2");
      }
    };

    requestAnimationFrame(updateActiveSection);
  }, [mainDescriptionInView, businessAreasInView]);
  ...
  const navigationItems = [
    { id: "section1", label: "Company Overview" },
    { id: "section2", label: "Business Areas" },
  ];
